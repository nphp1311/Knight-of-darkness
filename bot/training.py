...
def _color_label(color: str, locale: str = "vi") -> str:
    vi, en = COLOR_LABELS.get(color, (color, color))
...
async def run_tank_session(interaction, user):
    ...
    for turn in range(1, 6):
        ...
        label = _color_label(hint_color, locale)
        if locale == "en":
            text = (
                f"☠️ **Round {turn}/5** — The Bone Duelist is about to strike with **{label}**. "
                f"Which way do you dodge?\n\nPress the matching color to evade."
            )
        else:
            text = (
                f"☠️ **Lượt {turn}/5** — Đấu sĩ xương khô sắp **{label} tấn công ngươi**, "
